...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    if pretrained:
        ...
        model = torch.load('./checkpoint/yolov5s.pt').cuda()
    ...
    if opt.sync_bn and cuda and RANK != -1:
        qat_model = torch.nn.SyncBatchNorm.convert_sync_batchnorm(qat_model).to(device)
    ...
    if RANK in {-1, 0}:
        ...
        if not resume:
            ...
            qat_model.half().float()  # pre-reduce anchor precision

        callbacks.run('on_pretrain_routine_end')

    # DDP mode
    if cuda and RANK != -1:
        qat_model = smart_DDP(qat_model)
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            if RANK in {-1, 0}:
                ...
                callbacks.run('on_train_batch_end', ni, qat_model, imgs, targets, paths, plots)
