...
def nas_fix_backbone_match(yolo, pruned_yolo):
    ...
    for name, module in pruned_yolo.named_children():
        pruned_yolo_layer = []
        for submodule in module:
            pruned_yolo_layer.append(submodule)

    for name, module in yolo.named_children():
        
        for layerid in range(len(pruned_yolo_layer)):
            #print(module[layerid], type(module[layerid]))
            if isinstance(module[layerid], Conv):
                module[layerid] = conv_match(module[layerid], pruned_yolo_layer[layerid])
            
            elif isinstance(module[layerid], C3):
                module[layerid].cv1 = conv_match(module[layerid].cv1, pruned_yolo_layer[layerid].total.cv1)
                module[layerid].cv2 = conv_match(module[layerid].cv2, pruned_yolo_layer[layerid].total.cv2)
                module[layerid].cv3 = conv_match(module[layerid].cv3, pruned_yolo_layer[layerid].total.cv3)
                module[layerid].m = pruned_yolo_layer[layerid].total.m
    ...
    for name, model_type in yolo.named_modules():
        if isinstance(model_type, Conv):
            ...
            if pruned_yolo_layer.get(name) is None:
                ...
                print(name, pruned_yolo_layer[name], model_type.act)
